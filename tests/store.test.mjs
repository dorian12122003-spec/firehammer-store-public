...
test("homepage and Neon Orbit page render the public release", async () => {
  ...
  assert.match(page, /PRODUCTION\/STABLE/);
  assert.match(page, /Authenticode: unsigned/);
});

test("catalog is deterministic and data-driven", async () => {
  assert.equal(catalog.schema, "firehammer-store-catalog-v1");
...
test("Neon Orbit identity and hashes are exact", () => {
  ...
  assert.equal(game.publisher, "Shadow of the Moon Studios");
...
test("clean routes redirect to exact public GitHub Release assets", () => {
  for (const route of [
...
test("public export contains no secrets, local paths, source, or oversized files", async () => {
